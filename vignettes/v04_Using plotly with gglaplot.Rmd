...
The R package for plotly, an interactive plotting library, includes a function to convert ggplots into interactive ploty plots. A version of this is included in gglaplot which adjust the look of the plot to be more in keeping with the GLA IU style guide (if theme_gla() has been used on the ggplot).
...
The title and legend can look a bit crowded (the positioning of these outside the plot so they don't overlap is tricky). In some situations it might be better to write the plot title in the rmarkdown document instead and not include it in the plot itself.
