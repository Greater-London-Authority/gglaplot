...
```{r}
p_start <- plot + ggla_highlight(filter_type = 'start')
p_end <- plot + ggla_highlight(filter_type = 'end')
p_min <- plot + ggla_highlight(filter_type = 'min')
p_max <- plot + ggla_highlight(filter_type = 'max')
...
```{r}
plot +
  ggla_highlight(filter_type = 'xy', x_filt = ymd('2007-01-01'))
```

You can also change the `geom` of `ggla_highlight()` to add labels. This will probably require a bit of fiddling around with the `position_nudge()` function to get them in the right place. If we're labelling our data we might as well remove the legend.


```{r}
plot +
  theme_gla(legend = FALSE) +
  ggla_highlight(mapping = aes(label = location), filter_type = 'end', geom = 'text',
                 show.legend = FALSE, position = position_nudge(x = -6 , y = 0.5))
```
...
ggplot(data = LDNUK, mapping = aes(x = Year, y = GPG, group = location, 
                                   colour = location)) +
  # Add the ggla_line layer with an added aesthetic controlling the thickness of the lines
  ggla_line(aes(size = location)) +
  # Control the scaling of the thickness, want London (location = 1) to be thicker than UK (location = 2)
...
pal <- gla_pal(gla_theme = 'light', palette_type = 'categorical', n = numcats)
(ggplot(AvAtt,aes(x = Region, y = Score, fill = rev(Subject), label = round(Score,1))) +
  theme_gla() +
  ggla_horizbar() + 
  geom_text(position = position_stack(vjust = 0.85), color = gla_light$background) +
  theme(axis.text.x = element_blank()) +
  scale_fill_manual(values = pal,
                    breaks = rev(levels(AvAtt$Subject))) +
      labs(title = 'Average Attainment 8 Scores - 2016/17',
...
To plot geographical data, you can use the `ggla_sf()` which uses the `ggplot::geom_sf()` function and changes some of the defaults to fit with our style guide. This function works with *simple features data*, as used in the [sf package](https://www.rdocumentation.org/packages/sf/versions/0.7-2).



```{r}
pal <- gla_pal(gla_theme = 'light', palette_type = 'diverging', 
               main_colours = c('blue','red'), n = 3)

ggplot(EURef, aes(fill = CatLeave)) +
  ggla_sf() +
  scale_fill_manual(values = pal) +
...
(For some reason the labels are centred on this plots when rendered in markdown)
